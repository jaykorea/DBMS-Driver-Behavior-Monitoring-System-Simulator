...
class SerialTransmitter:
    ...
    def connectToMCU(self):
        ...
        if connectPort != 'None':
            self.link = txfer.SerialTransfer(connectPort, 9600)
    ...
    def printdata(self):
        print('Speed : %f ' % self.vehicle_speed)
        print('Throttle : %f ' % self.vehicle_throttle)
        print('Rpm : %f ' % self.vehicle_rpm)
        print('AccelX : %f ' % self.vehicle_accelX)
        print('AccelY : %f ' % self.vehicle_accelY)
        print('Latitude : %f ' % self.vehicle_lat)
        print('Longitude : %f ' % self.vehicle_lon)

    def store_data(self,world,p,v,c):
        # p = world.player.get_physics_control()
        # t = world.player.get_transform()
        # v = world.player.get_velocity()
        # c = world.player.get_control()
        if c.throttle == 0:
            c.throttle = 0.09
        engine_rpm = (p.max_rpm * (c.throttle) *  0.8)
        # if c.gear >= 0:
        ...
        self.vehicle_speed = float('%d' % (3.6 * math.sqrt(v.x**2 + v.y**2 + v.z**2)))
        self.vehicle_throttle = float('%f' % (float(c.throttle) * 100))
        self.vehicle_rpm = float('%f' % (engine_rpm))
        self.vehicle_accelX = float('%2.2f' % (world.imu_sensor.accelerometer[0] / 19.6))
        self.vehicle_accelY = float('%2.2f' % (world.imu_sensor.accelerometer[1] / 19.6))
        self.vehicle_lat = float('%2.3f' % (world.gnss_sensor.lat))
        self.vehicle_lon = float('%3.3f' % (world.gnss_sensor.lon))
...
if __name__ ==  '__main__':
    serialThread_ = threading.Thread(target=serialThread)
    serialThread_.daemon = True
    serialThread_.start()
    main()
